...
public class SecurityGrantRepositoryImpl implements SecurityGrantRepository {
    ...
    private static final String SQL_FIND_ALL_BY_SUBJECT_ID = "SELECT security_grant.id, security_grant.name from security_grant inner join subject_grant sg on security_grant.id = sg.grant_id where sg.subject_id = :id";
    private static final String SQL_FIND_ONE_BY_NAME = "SELECT security_grant.id, security_grant.name from security_grant where security_grant.name = :name";

    public SecurityGrantRepositoryImpl(JdbcTemplate jdbcTemplate, PlatformTransactionManager platformTransactionManager) {
    ...
    public List<SecurityGrant> findAllBySubjectId(Long subjectId) {
        return TransactionUtils.doInTransaction(
                ...
                (u) -> {
                    List<SecurityGrant> securityGrant = jdbcTemplate.query(SQL_FIND_ALL_BY_SUBJECT_ID, ROW_MAPPER, subjectId);
    ...
    public Optional<SecurityGrant> findOneByName(String name) {
        return TransactionUtils.doInTransaction(
                ...
                (u) -> {
                    SecurityGrant securityGrant = jdbcTemplate.queryForObject(SQL_FIND_ALL_BY_SUBJECT_ID, ROW_MAPPER, name);
